 ...
 * every time). Deliberately opt-in on every booking, new or existing: there
 * is no "suggested match" or automatic linking, only an explicit search or
 * an explicit "save as a new profile" — staff decide when two bookings are
 * really the same person, this never guesses.
 *
...
export default function GuestProfilePicker({
  ...
  /** Queued to create a brand-new profile from this booking's current fields on save. Mutually exclusive with linkedGuest. */
  createNew: boolean;
